-- Demo seed: ~80% booked across all matches.
BEGIN;
...
BEGIN
  FOR m IN SELECT id FROM public.matches WHERE cancelled_at IS NULL ORDER BY kickoff LOOP
    DECLARE
      ...
      methods public.payment_method[] := ARRAY['card','card','card','card','apple','google']::public.payment_method[];
      random_name text;
      ...
      u5_n int;
    BEGIN
      SELECT ARRAY(SELECT unnest(seats_pool) ORDER BY random()) INTO shuffled;
      WHILE filled < target AND i <= array_length(shuffled, 1) LOOP
        group_size := least(target - filled, 1 + (random()*4)::int);
        group_seats := shuffled[i:i+group_size-1];
        random_name := names[1 + (random()*(array_length(names,1)-1))::int];
        method := methods[1 + (random()*(array_length(methods,1)-1))::int];
        -- tier mix: mostly adult, sometimes concession/kid
        adult_n := group_size;
        conc_n := 0; u17_n := 0; u5_n := 0;
        IF group_size >= 2 AND random() < 0.4 THEN
          conc_n := 1;
          adult_n := adult_n - 1;
        END IF;
        IF group_size >= 3 AND random() < 0.3 THEN
          u17_n := 1;
          adult_n := adult_n - 1;
        END IF;
        IF group_size >= 4 AND random() < 0.2 THEN
          u5_n := 1;
          adult_n := adult_n - 1;
        END IF;
